...
?>
      ...
      <div class="sales-boxes">
        <div class="recent-sales box">
          <center>  <a href="rent.php"><button>Rent</button></a></center>
          <div class="title">Recent Sales</div>
          <div class="sales-details">
            <ul class="details">
              <li class="topic">user</li>
            <?php
              $user=mysqli_query($con,"SELECT * FROM rent WHERE 1");
              while($alluser=mysqli_fetch_array($user)){
                $sql=mysqli_query($con,"SELECT * FROM `usertable` WHERE `id`='$alluser[iduser]'");
                $result=mysqli_fetch_array($sql);
            ?>
            <li><a href="#"><?php echo$result['username'];?></a></li>
            <?php
                                        }?>
            </ul>
            <ul class="details">
              <li class="topic">user owner</li>
              <?php
              $user=mysqli_query($con,"SELECT * FROM rent WHERE 1");
              while($alluser=mysqli_fetch_array($user)){
                $sql=mysqli_query($con,"SELECT * FROM `book` WHERE `id`='$alluser[idbook]'");
                $result=mysqli_fetch_array($sql);
                $sqle=mysqli_query($con,"SELECT * FROM `usertable` WHERE `id`='$result[id_user]'");
                $resulte=mysqli_fetch_array($sqle);
                                            ?>
              <li><a href="#"><?php echo$resulte['username'];?></a></li>
              <?php
                                          }?>
            </ul>
            <ul class="details">
