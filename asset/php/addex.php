...
if(isset($_POST['exbook'])&& isset($_FILES['file'])){
            ...
            if ($errorimage === 0) {
              ...
              if (in_array($file_image_ex_lc, $img_image_exs)) {
                ...
                "INSERT INTO `book`(`page`, `name`, `nameclerk`, `dis`, `type`, `cost`, `email`, `data`, `time`, `id_user`, `statues`)
                 VALUES ('$number','$name','$clerk','$dis','$new_image_image_name','$cost','$email','$data','$time','$id','ex')");
      if($sql){
        header("Location: ../../user/users/addex.php?rn=The Add Succsfull");
      }else{
        header("Location: ../../user/users/addex.php?rn=The Add Not Succsfull");
      }
              }else{
        header("Location: ../../user/users/addex.php?rn=The Add Not Succsfull");
